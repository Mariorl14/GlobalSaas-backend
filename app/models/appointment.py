...
class Appointment(db.Model):
    ...
    proposed_start_time = db.Column(DateTime, nullable=True)
    proposed_end_time = db.Column(DateTime, nullable=True)
    previous_start_time = db.Column(DateTime, nullable=True)
    previous_end_time = db.Column(DateTime, nullable=True)
    reschedule_token = db.Column(db.String(64), nullable=True, unique=True, index=True)
    reschedule_token_expires_at = db.Column(DateTime, nullable=True)
    reschedule_message = db.Column(db.Text, nullable=True)
    cancel_reason = db.Column(db.String(40), nullable=True)
    cancel_message = db.Column(db.Text, nullable=True)
